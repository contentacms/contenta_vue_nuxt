...
export async function findOneRecipeByUuid(uuid) {
  ...
  return jsonApi.get('recipes', query, uuid);
}
...
export async function findHomePromotedArticlesAndRecipes(limit) {
  ...
  const promisesValues = await Promise.all([promotedRecipes, promotedArticles]);
...
export async function findAllRecipesByDifficultyName(
  difficultyName,
  limit = 4
) {
  const query = {
...
export async function findAllRecipesByMaxTotalTime(maxTotalTime, limit = 4) {
  const query = {
    sort: '-created',
    include: 'image,image.thumbnail',
    filter: {
