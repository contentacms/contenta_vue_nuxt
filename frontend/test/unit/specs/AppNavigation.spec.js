...
describe('AppNavigation.vue', () => {
  ...
  it('Mobile navigation should have a "is-active" classes when $store.state.menuMobileIsOpened == true', () => {
    expect(vm.$el.querySelector('.navbar-menu').className.trim()).toBe(
      'navbar-menu'
    );
    ...
    vm.$nextTick(() => {
      ...
      expect(vm.$el.querySelector('.navbar-burger').className.trim()).toBe(
  ...
  it('.navbar-burger "data-target" to be "app-menu" and to match .navbar-menu "id"', () => {
    ...
    expect(vm.$el.querySelector('.navbar-burger').dataset.target).toBe(
      navbarMenuId
    );
